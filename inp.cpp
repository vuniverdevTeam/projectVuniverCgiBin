...
double b(int* arr, double rating, int years)
{
  float alpha, beta;
  float B, F, x, y, function;
  float M = 0, D = 0;
  float* markArr = new float[years];
  int	i;
  int sum = 0;
  ...
  M /= years;
  for (i = 0; i < years; ++i) {
    D += pow(float(markArr[i] - M), float(2.0));
  }
  D /= years - 1;
  ...
  function = (1 / B) * pow(M, alpha - 1);
  function *= pow(1 - M, beta - 1);
...
short SQL :: searchMarkFS(const char *f, const char *s, int *arr)
{
  int i;
  char query_ptr[500];
  sprintf(query_ptr, "SELECT avg_contract_mark, avg_budget_mark FROM facult_spec WHERE s_id='%s' AND f_id='%s'", s, f);
  mysql_query(connection, query_ptr);
  res = mysql_store_result(connection);
...
Prop *SQL :: searchAllFS(int &n, const char *f, const char *s){
  ...
  double M, D;
  int marks[2*YEARS];
  mysql_query(connection, "SELECT f_id, s_id FROM coeff");
  ...
  for(n = 0; FSrow = mysql_fetch_row(FSres); n++){
    ...
    arr[n].prop2 = Probability(marks+YEARS, M, D);
  }
...
void Template :: Model(int dec)
{
  ...
  case 'C' :
    ...
    else cout<<b(arr, rating, YEARS)<<"%"<<endl;
    //cout<<"M: "<<M<<"<br/>"<<endl;
    //cout<<"D: "<<D<<endl;
    break;
  case 'B':
    ...
    else cout<<b((int*)(arr+YEARS), rating, YEARS)<<"%"<<endl;
    //cout<<"M: "<<M<<"<br/>"<<endl;
    //cout<<"D: "<<D<<endl;
    break;
  case 'c':
...
void Template :: Controller(char* file)
{
  ...
  if(isNorm){
    if(singleMode)
      fin.open(file);
    else fin.open("../project ISM/input2.html");

  ...
  if(strcmp(getenv("REQUEST_METHOD"),"GET") == 0)
  	Read();
        ...
        if(strcmp(en.arr[0].name, "sub") == 0){
          ...
          sprintf(condition, " WHERE u_id=%s", en.arr[0].value);
        }
        else if(strcmp(en.arr[0].name, "spec") == 0){
          strcat(table, "specialities JOIN facult_spec ON specialities.s_id=facult_spec.s_id");
          strcat(what, "specialities.s_id, specialities.s_name");
					if(*en.arr[0].value != '0' && en.arr[0].value != 0 && *en.arr[0].value != 0)
    	      sprintf(condition, " WHERE f_id=%s AND year=2005", en.arr[0].value);
					else	sprintf(condition, " WHERE year=2005");
        }
